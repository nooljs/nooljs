...
module.exports = function()
{
 ...
 monitorFiles=function()
 {
	...
	for(i=0; i < files.length; i++)
	{
		...
		if(stat.mtime > files[i].stat.mtime)
		{
			console.log(" file  :%s  has been modified.", files[i].fileName);
			paserLayout( fs, files[i].fileName, files[i].directoryName, null)
			.then(function(data)
			{
